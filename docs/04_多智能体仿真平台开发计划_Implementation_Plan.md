...
2. 添加WebSocket实时推送
3. 嵌入式存储服务集成 (Sled/SQLite-vec/petgraph)
...
| Vector DB | 先用 in-memory cosine similarity，后续切换 SQLite-vec |
| Graph DB | 先用 `petgraph` in-memory，后续切换 Embedded Graph Store |
| 沙箱引擎 | Rhai (已配置)，配合资源限制 |
| 快照存储 | 先用文件系统，后续可扩展 S3 |
| 注册存储 | SQLite (已配置 SQLx) |
...
#### 3. 外部服务集成 (2-4周)
- [ ] Embedded Hot Memory (修复API适配问题)
- [ ] SQLite-vec Vector Store (替换InMemoryVectorStore)
- [ ] Embedded Graph Store Graph Store (替换InMemoryGraphStore)
